...
class ConversationService {
  ...
  async handleGreetingResponse(conversation, input) {
    ...
    if (this.containsPositive(lowerInput)) {
      // Customer said yes, proceed to confirm interest
    ...
    } else if (this.containsNegative(lowerInput) || lowerInput.includes('busy') || lowerInput.includes('not now')) {
  ...
  containsPositive(input) {
    const positiveWords = ['yes', 'yeah', 'yep', 'sure', 'okay', 'ok', 'definitely', 'absolutely', 'interested', 'good time'];
    return positiveWords.some(word => input.includes(word));
  }

  containsNegative(input) {
    const negativeWords = ['no', 'nope', 'not', 'busy', 'later', 'bad time', 'not interested', 'not now'];
